...
while($count < $max_tries) {
  ...
  try {
      ...
      // If the text has more than 89 characters, then break
      if(strlen($schedule) > 89) {
        break;
      }
      
  } catch (Exception $e) { // Catch any exceptions
    $errors .= "Caught exception [iteration $count]: " . $e->getMessage() . "\n";
  }
    
}

// Hard-code calendar, since the site sometimes fails to load dynamic content
$schedule .= "\nhttps://www.cheshirewestandchester.gov.uk/asset-library/waste-calendars/Friday-A.PDF\n";

// Append counts
$plural = $count > 1 ? "s" : "";
$schedule .= "\n[$count attempt$plural]\n";
